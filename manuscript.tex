...
\subsection{捕捉近眼图像与眼动追踪数据}
Most near-eye eye trackers estimate gaze or vergence from extracting visible image features such as 
the pupil, iris, and corneal glints. 

However, these methods mainly describe eye rotation and provide limited information about accommodation 
and convergence, the key factors in the vergence--accommodation conflict (VAC)\cite{lu2020purkinje,nair2020riteyes}. 
...
另一方面，RIT-Eyes 说明了使用图形学渲染生成近眼图像数据的价值。该系统通过合成眼部图像来训练和测试眼动追踪网络，并加入了可变形虹膜、非球面角膜、视网膜回反射、眼睑运动和眨眼等因素 \cite{nair2020riteyes}. 不过，RIT-Eyes 更偏向生成带标注的训练数据，而本项目更关注 Purkinje 图像的物理形成过程。因此，我们的工作不是简单生成“看起来像眼睛”的图像，而是要显式建模角膜、晶状体、折射率、红外光源、相机位置和焦平面，以解释为什么某些设置下 P1 明显而 P3/P4 难以捕捉。
